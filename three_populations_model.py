...
# Initialize LFP signals for this parameter set
...
for trial in range(n_trials):
    ...
    factor_inter_exc_1=  0.5 #p_val  #0.3
    factor_inter_exc_2=  0.5 #1-p_val  #0.7
    ...
    excitatory_cell_params_1 = {
        ...
        "E_ex": 0.0,
        "E_in": -80.0,
        "E_L": variable_param(-70, 0.),  ## - 72 for manipulation
    ...
    inhibitory_cell_params_1 = {
        ...
        "E_ex": 0.0,
        "E_in": -80.0,
        "E_L": variable_param(-70.0, 0.),  ## - 72 for manipulation
    ...
    #pct = 0.25  # 1% decrease per trial (use 0.005 for 0.5%, etc.)
    pct = 0.0  # 0% decrease per trial
    ...
    inter_exc_exc_1 = factor_inter_exc_1 * 0.178
    inter_exc_exc_2 = factor_inter_exc_2 * 0.178
    ...
    nest.CopyModel("iaf_bw_2003_NMDA","exc_cell_3", excitatory_cell_params_2)
    nest.CopyModel("iaf_bw_2003_NMDA","inh_cell_3", inhibitory_cell_params_2)

    exc_cell_1 = nest.Create("exc_cell_1",NE)
    exc_cell_2 = nest.Create("exc_cell_2",NE)
    exc_cell_3 = nest.Create("exc_cell_3",NE)

    inh_cell_1=nest.Create("inh_cell_1",NI)
    inh_cell_2=nest.Create("inh_cell_2",NI)
    inh_cell_3=nest.Create("inh_cell_3",NI)
    ...
    nest.CopyModel("inhomogeneous_poisson_generator", "cortical_input_3")
    ...
    OU_x_3 = np.zeros(OU_n)  # OU output
    ...
    for i in range(OU_n - 1):
        ...
        OU_x_3[i + 1] = OU_x_3[i] + step * ((OU_mean_CC_2 - OU_x_3[i]) / OU_tau) + \
                      OU_sigma_bis * OU_sqrtdt * np.random.randn()
    ...
    cc_params_3 = {"rate_times": time_array, 'rate_values': OU_x_3}
    ...
    noise_cc_3 = nest.Create("cortical_input_3", params=cc_params_3)
    ...
    parrot_cc_3 = nest.Create("parrot_neuron",NE)
    ...
    nest.CopyModel("static_synapse", "inter_exc_exc_1",
                   {"weight": variable_weight(inter_exc_exc_1),
                    "delay": variable_delay(delay_ratio * delay)})

    nest.CopyModel("static_synapse", "inter_exc_exc_2",
                   {"weight": variable_weight(inter_exc_exc_2),
                    "delay": variable_delay(delay_ratio * delay)})
    ...
    nest.Connect(exc_cell_3, exc_cell_3, conn_params_1, "exc_exc_rec_2")
    ...
    nest.Connect(exc_cell_3, inh_cell_3, conn_params_1, "exc_inh_rec_2")
    ...
    nest.Connect(inh_cell_3, exc_cell_3, conn_params_1, "inh_exc_rec_2")
    ...
    nest.Connect(inh_cell_3, inh_cell_3, conn_params_1, "inh_inh_rec_2")
    ...
    nest.Connect(noise_cc_3, parrot_cc_3)
    ...
    nest.Connect(parrot_th, exc_cell_3,conn_params_1, syn_spec="exc_exc_thalamo")
    nest.Connect(parrot_th, inh_cell_3,conn_params_1, syn_spec="exc_inh_thalamo")
    ...
    nest.Connect(parrot_cc_3, exc_cell_3,conn_params_1, syn_spec="exc_exc_cortical")
    nest.Connect(parrot_cc_3, inh_cell_3,conn_params_1, syn_spec="exc_inh_cortical")


      ##  INTER populations ##

    nest.Connect(exc_cell_1, exc_cell_2, conn_params_inter, "inter_exc_exc_1")  ### FEEDFORWARD E-E
    nest.Connect(exc_cell_3, exc_cell_2, conn_params_inter, "inter_exc_exc_2")  ### FEEDFORWARD E-E
    ...
    print("Excitatory spikes 1:")
    print(f"Min neuron index: {sender_exc_spike_1.min()}")
    print(f"Max neuron index: {sender_exc_spike_1.max()}")

    print("Inhibitory spikes 1:")
    print(f"Min neuron index: {sender_inh_spike_1.min()}")
    print(f"Max neuron index: {sender_inh_spike_1.max()}")

    print("Excitatory spikes 2:")
    print(f"Min neuron index: {sender_exc_spike_2.min()}")
    print(f"Max neuron index: {sender_exc_spike_2.max()}")

    print("Inhibitory spikes 2:")
    print(f"Min neuron index: {sender_inh_spike_2.min()}")
    print(f"Max neuron index: {sender_inh_spike_2.max()}")
    ...
    for i in range(time_inh_spikes_1.size - 1):
        ind_i = int(sender_inh_spike_1[i + 1]) - 12000
    ...
    for i in range(time_inh_spikes_2.size - 1):
        ind_i = int(sender_inh_spike_2[i + 1]) - 13000
    ...
    # 3. CHANGED: Save trial results into the directory
    rate_file_path = os.path.join(dir_name, f'firing_rate_trace_trial{trial + 1}.csv')
    df_rates.to_csv(rate_file_path, index=False)
...
# 4. CHANGED: Save final LFP results into the directory after trial loop finishes
data_1 = pd.DataFrame(LFP_signal_1)
data_1.to_csv(os.path.join(dir_name, 'LFP_pop_1.csv'))

data_2 = pd.DataFrame(LFP_signal_2)
data_2.to_csv(os.path.join(dir_name, 'LFP_pop_2.csv'))

print(f"Finished processing for param: {p_val}")


######################### RASTER PLOT ###############################

# nest.raster_plot.from_device(espikes_1, hist=True)
# nest.raster_plot.from_device(espikes_2, hist=True)
